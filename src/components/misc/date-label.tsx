...
export const DateLabel = ({ date, originalDate }: { date: string, originalDate?: string }) => {
  ...
  const formattedActualDate = actualDateToFormat.toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' });

  if (originalDate) {
    const originalDateToFormat = new Date(originalDate);
    const formattedOriginalDate = originalDateToFormat.toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' });
    ...
    return (
      <Popover>
        ...
        <PopoverContent className="text-sm w-auto gap-2 flex flex-col p-4">
          <p>Published on <span className="font-semibold text-muted-foreground">{formattedActualDate}</span></p>
          <p>Originally published on <span className="font-semibold text-muted-foreground">{formattedOriginalDate}</span></p>
        </PopoverContent>
      </Popover>
    );
  }

  return <span className="font-[family-name:--title-font]"><span className="font-semibold">{formattedActualDate}</span></span>;
};
