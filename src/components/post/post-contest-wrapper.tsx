...
export function ContestPostWrapper({
  ...
  return (
    <div className={cn("relative rounded-lg border p-1 pt-3 mt-2 -ml-1 -mr-4", style.borderColor)}>
      <div
        className={cn(
      ...
      >
        ...
        <span className={style.prizeColor}>
          {place}
          {getOrdinalSuffix(place)} Place
        </span>
      </div>

      {prizeAmount && prizeTransactionHash && (
        <div className={cn("absolute -top-3 right-4 z-10", "bg-background border rounded-full", style.borderColor)}>
          <Link
      ...
      <div className={cn("absolute inset-0 rounded-lg bg-gradient-to-br opacity-30", style.bgGradient)} />
