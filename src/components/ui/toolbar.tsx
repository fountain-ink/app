    ...
    return typeof pressed === "boolean" ? (
      <ToolbarToggleGroup disabled={props.disabled} value="single" type="single">
        ...
        >
          {isDropdown ? (
            <>
              <div className="flex flex-1 items-center whitespace-nowrap">{children}</div>
              <div>
                <Icons.arrowDown className="ml-0.5 size-3.5 text-muted-foreground" data-icon />
              </div>
            </>
          ) : (
            children
...
export const ToolbarGroup = withRef<"div">(({ children, className }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("group/toolbar-group relative flex", className)}>
