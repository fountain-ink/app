...
>(({ children, icon, label, ...props }, ref) => {
  return (
    <Ariakit.MenuButton
      ref={ref as any}
      render={(children as any) ?? <MenuItem />}
      {...props}
    >
      {icon}
...
export const MenuContent = React.forwardRef<HTMLDivElement, MenuContentProps>(
  (
    { animateZoom, children, className, variant, onClickOutside, ...props },
    ref
  ) => {
    ...
    baseProps.children = (
      <>
        <div className="flex w-full items-center gap-2 whitespace-nowrap">
          {icon && (
            <span className="menu-item-icon text-subtle-foreground">
              {icon}
            </span>
          )}
        ...
        {(hasShortcut || checked) && (
          <div className="flex shrink-0 items-center justify-end">
            ...
            {hasShortcut && (
              <MenuShortcut>
                {shortcut ?? (
                  <CornerDownLeft className="hidden group-data-[active-item='true']/menu-item:inline-block" />
                )}
              </MenuShortcut>
            )}
            {checked && searchable && (
...
export const ComboboxInput = withRef<typeof Ariakit.Combobox>(
  ({ children, ...props }, ref) => {
    return (
      <Ariakit.Combobox
        ref={ref}
