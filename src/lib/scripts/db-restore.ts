...
async function restoreDatabase(options: RestoreOptions) {
  ...
  try {
    ...
    psqlArgs.push(
      "--file", tempSchemaPath,  // Use processed schema
      "--command", "SET session_replication_role = replica",
      "--file", join(selectedBackup, "data.sql"),
    ...
    if (isLocal) {
      ...
      const checkProc = spawn({
        cmd: ["psql", connectionUrl, "-t", "-c", "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';"],
        stdout: "pipe",
        stderr: "pipe"
      })
      
      const checkOutput = await new Response(checkProc.stdout).text()
      const tableCount = parseInt(checkOutput.trim()) || 0
