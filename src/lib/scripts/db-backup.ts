...
async function backupDatabase() {
  ...
  try {
    ...
    // Backup roles
    const rolesSuccess = await runSupabaseBackup(env.DATABASE_URL, `${backupDir}/roles.sql`, ["--role-only"]);

    // Backup schema
    const schemaSuccess = await runSupabaseBackup(env.DATABASE_URL, `${backupDir}/schema.sql`, []);

    // Backup data
    const dataSuccess = await runSupabaseBackup(env.DATABASE_URL, `${backupDir}/data.sql`, [
      "--use-copy",
      "--data-only",
    ]);
